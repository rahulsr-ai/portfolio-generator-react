...
const EditPortfolioPage = () => {
    ...
    const fetchPortfolio = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          // Map social media array to object
          const socialsObj = { linkedin: "", github: "", twitter: "" };
          data.socialMedia?.forEach((social) => {
            if (social.platform && social.url) {
              socialsObj[social.platform.toLowerCase()] = social.url;
            }
          });

          // Ensure arrays have exactly 3 items
          const ensureThreeItems = (arr, emptyItem) => {
          ...
          setFormData({
            ...
            socials: socialsObj,
            skills: data.skills || [],
            services: ensureThreeItems(data.services, {
              title: "",
              description: "",
            }),
            testimonials: ensureThreeItems(data.testimonials, {
              name: "",
              quote: "",
            }),
            blogs: ensureThreeItems(data.blogs, { title: "", summary: "" }),
            projects: ensureThreeItems(
              data.projects?.map((p) => ({
                title: p.title,
                image: null,
                imageUrl: p.image,
            ...
            contactMessage: data.contactMsg || "",
            contactEmail: data.contactEmail || "",
            contactPhone: data.contactPhoneNo?.toString() || "",
          });
        }
      } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_URL}/api/portfolio/update/${params.id}`,
        {
          method: "PUT",
          ...
          <div className="flex justify-between mt-6 pt-4 border-t border-gray-200">
            ...
            {currentStep === steps.length ? (
              ...
              >
                ...
                <span>{isUploading ? "Updating..." : "Update Portfolio"}</span>
              </button>
            ) : (
              <button
                onClick={nextStep}
                className="px-4 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition-colors text-sm md:text-base"
              >
                Next Step
              </button>
            )}
